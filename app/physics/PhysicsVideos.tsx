...
interface PhysicsVideo {
  id: string;
  title: string;
  youtubeId: string;
}

function YouTubeCard({ video }: { video: PhysicsVideo }) {
        ...
        {playing ? (
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-red-600 shadow-xl shadow-red-600/30 transition-transform duration-300 group-hover:scale-110">
                <Play className="h-6 w-6 fill-white text-white mr-[-2px]" />
              </div>
            </div>
          </button>
        )}
...
export default function PhysicsVideos({ videos }: { videos: PhysicsVideo[] }) {
    ...
    <>
      ...
      <p className="mt-4 text-sm text-text-muted">
        {filtered.length === videos.length ? (
          <>
            <span className="num font-semibold text-text">{videos.length}</span> סרטונים
          </>
        ) : (
          <>
